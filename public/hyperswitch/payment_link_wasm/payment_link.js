...
export function validate_payment_link_config(config_json) {
  ...
  try {
    ...
    const ret = wasm.validate_payment_link_config(ptr0, len0);
    var ptr2 = ret[0];
    var len2 = ret[1];
    if (ret[3]) {
      ptr2 = 0;
...
export function generate_payment_link_preview(config_json) {
  ...
  try {
    ...
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.generate_payment_link_preview(ptr0, len0);
    var ptr2 = ret[0];
    var len2 = ret[1];
    if (ret[3]) {
      ptr2 = 0;
...
async function __wbg_init(module_or_path) {
  ...
  if (typeof module_or_path === "undefined") {
    module_or_path = new URL("payment_link_bg.wasm", import.meta.url);
  }
  const imports = __wbg_get_imports();
