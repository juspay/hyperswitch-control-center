...
- **Action:** Add a new atom for your page's table.
  ...
  _Ensure `MyNewPageEntity.defaultColumns` is accessible. You might need to add `open MyNewPageEntity` at the top of `TableAtoms.res` or qualify it fully if not already done for other entities._
...
- **Mapper Signature for `LogicUtils.getArrayDataFromJson`:**
  ...
  - **Correct `itemToObjMapper` definition:**
    ...
    ```
  - **Common Mistake:** Defining `itemToObjMapper` as `(jsonItem: JSON.t) => ...` and then trying to convert `jsonItem` to a dictionary _inside_ the mapper. While this works for the `LogicUtils.getString` calls, it makes the mapper incompatible with `LogicUtils.getArrayDataFromJson`.
