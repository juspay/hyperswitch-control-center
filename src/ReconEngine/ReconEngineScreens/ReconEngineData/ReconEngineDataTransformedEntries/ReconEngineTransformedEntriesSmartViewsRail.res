...
module Tab = {
  @react.component
  let make = (
    ~view: entriesSmartView,
    ~active: bool,
    ~count: int,
    ~onSelect: entriesSmartView => unit,
  ) => {
    ...
    <button
      ...
      <span className={`${body.sm.medium} whitespace-nowrap`}>
        {view->entriesSmartViewLabel->React.string}
      </span>
      <span
...
@react.component
let make = (
  ~activeView: entriesSmartView,
  ~onChange: entriesSmartView => unit,
  ~counts: array<(entriesSmartView, int)>,
) => {
  let countFor = (view: entriesSmartView) =>
    counts
  ...
  <div
    ...
    {allEntriesSmartViews
    ->Array.map(view =>
      <Tab
        key={view->entriesSmartViewLabel}
        view
