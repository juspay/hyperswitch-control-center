...
let useCreateIngestionConfig = () => {
  ...
  async (
    ...
    try {
      ...
      showToast(~message=`Ingestion config "${name}" created successfully`, ~toastType=ToastSuccess)
      Some(created)
    } catch {
    | Exn.Error(e) => {
...
let useCreateTransformationConfig = () => {
  ...
  async (~form: transformationFormState, ~merchantId: string, ~profileId: string) => {
    if form.name->String.trim->String.length === 0 || form.accountId->String.length === 0 || form.ingestionId->String.length === 0 {
      showToast(~message="Name, account, and ingestion source are required", ~toastType=ToastError)
      None
    } else if form.currencyIdentifier->String.trim->String.length === 0 || form.amountIdentifier->String.trim->String.length === 0 || form.effectiveAtIdentifier->String.trim->String.length === 0 || form.orderIdIdentifier->String.trim->String.length === 0 {
      showToast(~message="Currency, amount, date, and order ID column mappings are required", ~toastType=ToastError)
      None
    } else {
    try {
      let url = getURL(
...
let useCreateReconRule = () => {
  ...
  async (~form: ruleFormState) => {
    if form.ruleName->String.trim->String.length === 0 || form.sourceAccountId->String.length === 0 || form.targetAccountId->String.length === 0 {
      showToast(~message="Rule name, source account, and target account are required", ~toastType=ToastError)
      None
