...
module AccountCard = {
  ...
  let make = (~account, ~index) => {
    ...
    let accountData = account->getDictFromJsonObject->accountItemToObjMapper
    let iconName = getAccountIcon(index)
    let handleCardClick = _ => {
      RescriptReactRouter.replace(
        GlobalVars.appendDashboardPath(~url=`v1/recon-engine/overview/${accountData.account_id}`),
      )
    }
    <div
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className={`${body.lg.semibold} text-gray-800`}>
          {accountData.account_name->React.string}
        </div>
      </div>
      <div className="grid grid-cols-2 gap-6 mb-4">
        <div>
          <div className={`${body.sm.regular} text-gray-500 mb-1`}>
            {"Currency"->React.string}
          </div>
          <div className={`${body.md.medium} text-gray-900`}>
            {accountData.currency->React.string}
          </div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-6 mb-4">
        <div>
          ...
          <div className={`${body.md.medium} text-gray-900`}>
            {accountData.posted_balance->React.string}
          </div>
        </div>
        <div>
          <div className={`${body.sm.regular} text-gray-500 mb-1`}>
          ...
          <div className={`${body.md.medium} text-gray-900`}>
            {accountData.pending_balance->React.string}
          </div>
        </div>
      </div>
    </div>
...
let make = () => {
  ...
  <PageLoaderWrapper screenState>
    <div className="flex flex-col gap-6 w-full">
      ...
      <div>
        <div className={`${body.lg.semibold} text-gray-800 mb-2`}> {"Accounts"->React.string} </div>
        <div className={`${body.md.regular} text-gray-600 mb-6`}>
