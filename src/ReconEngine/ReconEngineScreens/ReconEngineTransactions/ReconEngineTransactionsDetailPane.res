  ...
  let make = () =>
    <div className="flex flex-1 flex-col items-center justify-center text-center px-8 gap-3">
      ...
      <p className={`${body.lg.semibold} text-nd_gray-600`}>
        {"Select a transaction"->React.string}
      </p>
      <p className={`${body.sm.medium} text-nd_gray-400 max-w-xs`}>
        {"Pick a row on the left to see its full reconciliation context here."->React.string}
      </p>
    </div>
}
  ...
  let make = (~onOpenFull: unit => unit, ~onClose: unit => unit) =>
    <div
      ...
      <span className={`${body.sm.medium} text-nd_gray-500 flex-1`}>
        {"Transaction details"->React.string}
      </span>
      <button
        type_="button"
...
module HeroBlock = {
  @react.component
  let make = (~txn: transactionType, ~onOpenFull: unit => unit) => {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-row items-center gap-2.5 flex-wrap">
        ...
        <span className={`${body.sm.medium} text-nd_gray-500`}>
          {`Recorded ${txn.created_at->formatRelativeTime} ago`->React.string}
        </span>
      </div>
        ...
        <div className="flex flex-row items-baseline gap-2 flex-wrap">
          ...
          | MismatchKind =>
            <span
              ...
              {`±${currency} ${variance->Float.toString} variance`->React.string}
            </span>
          | _ => React.null
          }}
        </div>
...
module LinkedSection = {
  @react.component
  let make = (~linked: linkedTransactionType) => {
    let isArchived = switch linked.transaction_status {
    | Archived => true
    | _ => false
    }
    <div className="flex flex-col gap-2.5">
      <span className={`${body.xs.semibold} text-nd_gray-400 uppercase tracking-wider`}>
        {"Linked with"->React.string}
      </span>
      <div
        className="rounded-xl border border-nd_primary_blue-100 bg-nd_primary_blue-50/40 px-3.5 py-3 flex flex-row items-center gap-3">
        <div className="flex flex-col gap-1 min-w-0 flex-1">
          <span className={`${body.xs.semibold} text-nd_primary_blue-600 uppercase tracking-wider`}>
            {"Linked transaction"->React.string}
          </span>
          <HelperComponents.CopyTextCustomComp
            customTextCss={`${body.sm.medium} text-nd_gray-700 font-mono truncate`}
        ...
        <RenderIf condition=isArchived>
          <span
            className={`${body.xs.semibold} px-2 py-0.5 rounded bg-nd_gray-150 text-nd_gray-600 uppercase tracking-wider flex-shrink-0`}>
...
module EntryRow = {
  @react.component
  let make = (~entry: transactionEntryType) => {
    ...
    <div
      className="flex flex-row items-center gap-3 pt-3 border-t border-dashed border-nd_gray-150">
      <span
        className={`${body.xs.semibold} ${textColor} pl-2 border-l-[3px] ${stripe} tracking-wider flex-shrink-0`}>
        {label->React.string}
      </span>
      <span className={`${body.sm.medium} text-nd_gray-700 font-mono truncate flex-1 min-w-0`}>
...
module AccountEntry = {
  @react.component
  let make = (~accountName: string, ~entries: array<transactionEntryType>) =>
    <div className="rounded-xl border border-nd_gray-150 bg-white px-4 py-3.5">
      <div className="flex flex-row items-baseline justify-between mb-2.5">
        <span className={`${body.sm.semibold} text-nd_gray-600 uppercase tracking-wider truncate`}>
          {accountName->React.string}
        </span>
      </div>
      {entries
      ->Array.mapWithIndex((entry, idx) =>
        <EntryRow key={entry.entry_id ++ idx->Int.toString} entry />
      )
      ->React.array}
    </div>
}
    ...
    <div className="flex flex-col gap-2.5">
      ...
      <div className="flex flex-col gap-2.5">
        ...
        ->Array.map(((accountId, entries)) => {
          ...
          <AccountEntry key={accountId} accountName entries />
        })
        ->React.array}
      </div>
    </div>
...
@react.component
let make = (
  ~accounts as _: array<accountType>,
  ~activeTransaction: option<transactionType>,
  ~onClearSelection: ReactEvent.Mouse.t => unit,
) => {
  let openFull = (txn: transactionType) =>
    RescriptReactRouter.push(
      GlobalVars.appendDashboardPath(~url=`/v1/recon-engine/transactions/${txn.transaction_id}`),
    )
