let setData = (offset, setOffset, total, data, setTotalCount, setTableData, setScreenState) => {
  ...
  if total > 0 {
    ...
    let orderData =
      arr->Array.concat(dataDictArr)->Array.map(RefundsTableEntity.tableItemToObjMapper)
...
let getData = async (
  ...
  try {
    let url = "https://sandbox.hyperswitch.io/analytics/v1/search/refunds"
...
let make = () => {
  ...
  <ErrorBoundary>
    <div className={`flex flex-col mx-auto h-full ${widthClass} ${heightClass} min-h-[50vh]`}>
      <PageUtils.PageHeading title="Refunds" customTitleStyle />
