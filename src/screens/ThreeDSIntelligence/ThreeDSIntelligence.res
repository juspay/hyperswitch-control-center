...
module ActiveRulePreview = {
  ...
  let make = (~initialRule, ~setInitialRule) => {
    ...
    let ruleInfo =
      ...
      ->AdvancedRoutingUtils.ruleInfoTypeMapper(~isFrom3dsIntelligence=true)
    ...
    <div className="relative flex flex-col gap-6 w-full border p-6 bg-white rounded-md">
      ...
      <RulePreviewer ruleInfo isFrom3ds=false isFrom3dsIntelligence=true />
    </div>
  }
}
...
let make = () => {
  ...
  let activeRoutingDetails = async () => {
    open LogicUtils
    try {
      let threeDsUrl = getURL(~entityName=V1(THREE_DS_INTELLIGENCE_LIST), ~methodType=Get)
      ...
      let activeRulesUrl = getURL(
        ~entityName=V1(THREE_DS_INTELLIGENCE_ACTIVE_RULE),
        ~methodType=Get,
        ~id=Some(ruleId),
      )
      ...
      let programValue = responseDict->getObj("algorithm", Dict.make())->getObj("data", Dict.make())
  ...
  let onSubmit = async (values, _) => {
    try {
      ...
      let threeDsPayload = valuesWithProfileId->JSON.Encode.object->buildThreeDsPayloadBody
      let getActivateUrl = getURL(~entityName=V1(THREE_DS_INTELLIGENCE), ~methodType=Put)
      ...
      let activateUrl = getURL(
        ~entityName=V1(ACTIVATE_3DS_RULE),
        ~methodType=Post,
        ~id=Some(routingId),
      )
      ...
      fetchDetails()->ignore
      RescriptReactRouter.replace(GlobalVars.appendDashboardPath(~url="/3dsintelligence"))
      setPageView(_ => LANDING)
      setScreenState(_ => Success)
  ...
  let redirectToNewRule = () => {
    setPageView(_ => NEW)
    RescriptReactRouter.replace(GlobalVars.appendDashboardPath(~url="/3dsintelligence?type=new"))
  }
