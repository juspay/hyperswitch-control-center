module OrgMerchantSwitchCollapsed = {
  @react.component
  let make = () => {
    let {orgId, merchantId} = React.useContext(UserInfoProvider.defaultContext)
...
let make = (~isExpanded=false) => {
  let expandedContent =
    <div className="flex flex-col items-end gap-2">
      <OrgSwitch />
      <div className="flex">
        <div className="w-6 h-10 border-blue-810 ml-10 border-dashed border-b border-l rounded" />
        <MerchantSwitch />
      </div>
    </div>
