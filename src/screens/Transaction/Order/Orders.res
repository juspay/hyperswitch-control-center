...
let make = (~previewOnly=false) => {
  ...
  let {merchantId, profileId, orgId, version} = getCommonSessionDetails()
  let paymentListRequestScopeKey = [merchantId, profileId, orgId]->Array.joinWith(":")
  ...
  let savedViewsVersion: UserInfoTypes.version = isAdvancedSource ? V2 : V1
  ...
  let filtersUI = React.useMemo(() => {
    ...
    let savedViewsAction = devSavedViews
      ? <SavedViewsComponent version=savedViewsVersion entity=SavedViewTypes.Payment />
      : React.null
    ...
    <RemoteTableFilters
      ...
      version
    />
  }, (searchText, version, tableTitle, isAdvancedSource, savedViewsVersion, devSavedViews))
  ...
  let checkBoxProps = isAdvancedSource
    ? Some({
        ...
        clearSelectionOnPartialSelectAll: true,
        expandCheckboxClickArea: true,
      })
    : None
  ...
  <ErrorBoundary>
    <div
      ...
      <RenderIf condition={transactionView && paymentListSourceResolved}>
        {isAdvancedSource
          ? <AdvancedPaymentTransactionView version requestScopeKey=paymentListRequestScopeKey />
          : <div className="grid lg:grid-cols-6 md:grid-cols-3 sm:grid-cols-3 grid-cols-2 gap-6">
