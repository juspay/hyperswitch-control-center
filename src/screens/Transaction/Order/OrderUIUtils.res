...
let copyAdvancedPaymentFilterIfPresent = (~fromDict, ~toDict, key) => {
  ...
  | Some(value) =>
    ...
    if key === customerEmailFilterKey {
      ...
      | Some(email) =>
        let isValidEmail = !(email->CommonAuthUtils.isValidEmail)
        if isValidEmail {
...
let buildAdvancedPaymentListPayload = (
  ...
  if trimmedSearchText->isNonEmptyString {
    let isValidEmailSearch = !(trimmedSearchText->CommonAuthUtils.isValidEmail)
    let isPaymentIdSearch = trimmedSearchText->String.startsWith("pay_")
    let isCardLast4Search = RegExp.test(%re("/^\d{4}$/"), trimmedSearchText)

    if isValidEmailSearch {
      body->setOptionString(customerEmailFilterKey, Some(trimmedSearchText))
    } else if isPaymentIdSearch {
      body->setOptionString(paymentIdFilterKey, Some(trimmedSearchText))
    } else if isCardLast4Search {
