...
let useOMPData = () => {
  ...
  let getNameForId = entityType =>
    switch entityType {
    | #Organization => currentOrgName(orgList, userInfo.orgId)
    | #Merchant => currentMerchantName(merchantList, userInfo.merchantId)
    | #Profile => currentProfileName(profileList, userInfo.profileId)
    | _ => ""
    }
