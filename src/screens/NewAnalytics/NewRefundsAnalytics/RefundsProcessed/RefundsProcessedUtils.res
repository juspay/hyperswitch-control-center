...
let getHeading = colType => {
  ...
  | Refund_Success_Count =>
    Table.makeHeaderInfo(
      ~key=Refund_Success_Count->getStringFromVariant,
      ~title="Count",
      ~dataType=TextType,
    )
...
let getCell = (obj, colType): Table.cell => {
  ...
  | Refund_Success_Count => Text(obj.refund_success_count->Int.toString)
...
let dropDownOptions = [
  ...
  {label: "By Count", value: Refund_Success_Count->getStringFromVariant},
]
...
let getMetaDataMapper = key => {
  ...
  | Refund_Success_Count | _ => Total_Refund_Processed_Count
...
let modifyQueryData = data => {
  ...
  data->Array.forEach(item => {
    ...
    | Some(prevVal) => {
        let key = Refund_Success_Count->getStringFromVariant
        ...
        prevVal->Dict.set(
          Refund_Success_Count->getStringFromVariant,
