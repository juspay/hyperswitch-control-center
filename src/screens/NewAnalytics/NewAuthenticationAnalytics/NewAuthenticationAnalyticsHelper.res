...
module OverViewStat = {
  ...
  let make = (~responseKey, ~data, ~getInfo, ~getValueFromObj, ~getStringFromVariant) => {
    ...
    let currency =
      filterValueJson->getString((#currency: NewAuthenticationAnalyticsTypes.filters :> string), "")
    ...
    let (value, direction) = NewAuthenticationAnalyticsUtils.calculatePercentageChange(
      ~primaryValue,
      ~secondaryValue,
    )
