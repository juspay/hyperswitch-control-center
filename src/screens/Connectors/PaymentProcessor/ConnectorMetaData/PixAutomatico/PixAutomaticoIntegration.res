@react.component
let make = (~connector, ~metadataKey, ~closeAccordionFn, ~update, ~onCloseClickCustomFun) => {
  open LogicUtils
  open PixAutomaticoUtils
  open Typography
    ...
    ->Array.mapWithIndex((field, index) => {
      ...
      <div key={`${inputField.name}_${index->Int.toString}`}>
        <FormRenderer.FieldRenderer
          ...
          field={pixAutomaticoFieldInput(
            ~metadataKey,
            ~pixAutomaticoField=inputField,
  ...
  <div className="flex flex-col gap-6 p-6">
    ...
    <div className={`flex gap-2 justify-end`}>
      ...
      <Button
        ...
        buttonState={formState.values->validatePixAutomaticoFields(~metadataKey)}
      />
    </div>
  </div>
}
