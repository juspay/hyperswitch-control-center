let textInput = (~field: CommonDataTypes.inputField, ~formName) => {
...
let selectInput = (
  ~field: CommonDataTypes.inputField,
  ~formName,
  ~opt=None,
...
let multiSelectInput = (~field: CommonDataTypes.inputField, ~formName) => {
...
let toggleInput = (~field: CommonDataTypes.inputField, ~formName) => {
  let {label} = field
  FormRenderer.makeFieldInfo(
    ~name={formName},
    ~label,
