...
let webhookVersion = FormRenderer.makeFieldInfo(
  ...
  ~placeholder="Enter webhooks version",
...
let webhookUsername = FormRenderer.makeFieldInfo(
  ...
  ~placeholder="Enter webhooks username",
  ~customInput=InputFields.textInput(~autoComplete="off", ()),
  ~isRequired=false,
  (),
)

let createdPaymentToggle = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="payment_created_enabled",
  ~customInput=InputFields.boolInput(~isDisabled=false),
  ~isRequired=false,
  (),
)

let succeededPaymentToggle = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="payment_succeeded_enabled",
  ~customInput=InputFields.boolInput(~isDisabled=true),
  ~isRequired=false,
  (),
)

let failedPaymentToggle = FormRenderer.makeFieldInfo(
  ~label="",
...
let primaryContactPerson = FormRenderer.makeFieldInfo(
...
let secondaryContactPerson = FormRenderer.makeFieldInfo(
...
let addressL1 = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="line1",
  ~placeholder="",
  ~customInput=InputFields.textInput(),
...
let addressL2 = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="line2",
  ~placeholder="",
  ~customInput=InputFields.textInput(),
...
let addressL3 = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="line3",
  ~placeholder="",
  ~customInput=InputFields.textInput(),
...
let addressCity = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="city",
  ~placeholder="City",
  ~customInput=InputFields.textInput(),
...
let addressState = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="state",
  ~placeholder="State",
  ~customInput=InputFields.textInput(),
...
let addressZip = FormRenderer.makeFieldInfo(
  ~label="",
  ~name="zip",
  ~placeholder="Zip",
  ~customInput=InputFields.textInput(),
...
let primaryEmail = FormRenderer.makeFieldInfo(
  ~label="Primary Email",
  ~name="primary_email",
...
let secondaryEmail = FormRenderer.makeFieldInfo(
...
let primaryPhone = FormRenderer.makeFieldInfo(
  ~label="Primary Phone",
  ~name="primary_phone",
...
let secondaryPhone = FormRenderer.makeFieldInfo(
  ~label="Secondar Phone",
...
let validateChangePasswordForm = (
  values: Js.Json.t,
  ~setIsDisabled=_ => (),
  keys: array<string>,
) => {
  ...
  | Some(dict) =>
    dict
    ->Js.Dict.entries
    ->Array.reduce(Js.Dict.empty(), (acc, entry) => {
      let (key, value) = entry
      switch value->Js.Json.decodeString {
      | Some(strVal) => Js.Dict.set(acc, key, strVal->Js.Json.string)
      | None => ()
      }
      acc
    })
  ...
  keys->Js.Array2.forEach(key => {
    ...
    if value == "" {
      switch key {
      | "oldPassword" => Js.Dict.set(errors, key, "Please enter your Old Password"->Js.Json.string)
      | "password" => Js.Dict.set(errors, key, "Please enter your New Password"->Js.Json.string)
      | "comfirmPassword" =>
        Js.Dict.set(errors, key, "Please enter your New Password Once Again"->Js.Json.string)
      | _ =>
        Js.Dict.set(
          errors,
          key,
    ...
    // password check
    passwordKeyValidation(value, key, "password", errors)

    // confirm password check
    confirmPasswordCheck(value, key, "comfirmPassword", "password", valuesDict, errors)
  })
...
let convertJsontoDict = (values: Js.Json.t) => {
  ...
  | Some(dict) =>
    ...
    ->Array.reduce(Js.Dict.empty(), (acc, entry) => {
      ...
      | "primary_business_details" =>
        ...
        | Some(strVal) => Js.Dict.set(acc, key, strVal->Js.Json.array)
        | None => ()
        }
      | _ =>
        switch value->Js.Json.decodeString {
