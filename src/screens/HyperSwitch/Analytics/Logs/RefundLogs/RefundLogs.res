...
let make = (~refundID) => {
  ...
  let fetchPaymentLogsData = async _ => {
    try {
      let paymentLogsUrl = getURL(~entityName=PAYMENT_LOGS, ~methodType=Get, ~id=Some(refundID), ())
  ...
  let getDetails = async () => {
    if !(refundID->HSwitchOrderUtils.isTestPayment) {
  ...
  <PageLoaderWrapper
    ...
    {if refundID->HSwitchOrderUtils.isTestPayment || logs.contents->Array.length === 0 {
      <div
