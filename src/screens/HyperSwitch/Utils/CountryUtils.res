...
let getCountryFromCountryCode = code => {
  switch code {
  | AF => Afghanistan
  | AX => AlandIslands
  | AL => Albania
  ...
  | LU => Luxembourg
  | MO => Macao
  | MK => MacedoniaTheFormerYugoslavRepublic
  | MG => Madagascar
  | MW => Malawi
  | MY => Malaysia
  | MV => Maldives
...
let getCountryFromString = country => {
  ...
  | "Zambia" => Zambia
  | "Zimbabwe" => Zimbabwe
  | _ => UnitedStatesOfAmerica
  }
}
let getCountryCodeFromString = code => {
  switch code {
  | "AF" => AF
  | "AX" => AX
  | "AL" => AL
