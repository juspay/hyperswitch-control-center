...
let make = () => {
  ...
  <div className="flex flex-1 flex-col gap-8 w-full h-screen items-center">
    ...
    <div className="flex gap-4 w-1034-px">
      ...
      ->Array.map(item =>
        <DefaultActionItem heading=item.heading description=item.description img=item.imgSrc />
      )
      ->React.array}
    </div>
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        ->Array.map(item =>
          <DefaultHomeCard
            heading=item.heading description=item.description img=item.imgSrc route=item.route
          />
        )
        ->React.array}
      </div>
