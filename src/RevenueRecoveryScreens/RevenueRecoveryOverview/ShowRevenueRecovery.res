...
let make = (~id) => {
  ...
  let dataRecoveryType = RevenueRecoveryEntity.itemToObjMapper(res->getDictFromJsonObject)

  let statusUI = useGetStatus(dataRecoveryType)
    ...
    <div className="flex flex-col gap-10">
      <div className="flex flex-row justify-between items-center">
        <div className="flex gap-2 items-center">
          <PageUtils.PageHeading title={`${dataRecoveryType.invoice_id}`} />
          {statusUI}
        </div>
      ...
      <div className="grid grid-cols-4  ">
        <div className="col-span-3">
          <OrderInfo order=dataRecoveryType />
        </div>
