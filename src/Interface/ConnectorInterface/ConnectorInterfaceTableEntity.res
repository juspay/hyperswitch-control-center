...
let getPreviouslyConnectedListV1 = (json: JSON.t): array<connectorPayloadCommonType> => {
...
let getPreviouslyConnectedListV2 = (json: JSON.t): array<connectorPayloadCommonType> => {
  ConnectorInterface.mapJsonArrayToConnectorPayloads(
...
let getConnectorObjectFromListViaId = (
  ...
  | V1 => ConnectorInterface.connectorInterfaceV1
  | V2 => ConnectorInterface.connectorInterfaceV2
  }
  connectorList
  ->Array.find(ele => {ele.id == mca_id})
  ->Option.getOr(ConnectorInterface.mapDictToConnectorPayload(interface, Dict.make())) //interface
}
