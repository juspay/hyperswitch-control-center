open LogicUtils
open PaymentInterfaceAnalyticsTypes
...
let getAnalyticsRow = (json: JSON.t): analytics_row => {
  let dict = json->getDictFromJsonObject
  {
    ...
    payment_method_data: Some(dict->getJsonObjectFromDict("payment_method_data")),
...
let mapAnalyticsRowToCommonType = (row: analytics_row): PaymentInterfaceTypes.order => {
  let activeAttempt =
    ...
    ->Option.getOr(Dict.make()->analyticsAttemptItemToObjMapper)
  ...
  let amount_capturable = activeAttempt.amount_capturable
  // Date Conversion
...

  {
    ...
    payment_method_data: row.payment_method_data,
    external_authentication_details: Some(
      row.external_authentication_details
      ->Option.getOr("")
      ->JSON.Encode.string,
    ),
