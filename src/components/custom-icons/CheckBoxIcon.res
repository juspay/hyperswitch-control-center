...
let make = (
  ...
  let cursorClass = isDisabled ? "cursor-not-allowed" : "cursor-pointer"

  <AddDataAttributes
    attributes=[("data-selected-checkbox", isSelected ? "Selected" : "NotSelected")]>
    {if isDisabled {
      if isSelected {
        <div className={`${checkboxDimension} ${cursorClass}`}>
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
        ...
        <div className={`${checkboxDimension} ${cursorClass}`}>
          <svg
            ...
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <rect width="16" height="16" rx="4" fill="#ECEFF3" />
            <rect
      ...
      if isSelected {
        <div className={`${checkboxDimension} ${cursorClass}`} onClick>
        ...
        <div className={`${checkboxDimension} ${checkboxBorderClass} ${cursorClass}`} onClick />
      }
    } else {
      <div
        className={`${checkboxDimension} ${!isSelected ? checkboxBorderClass : ""} ${cursorClass}`}
        onClick>
