...
let make = (
  ...
  let (date, setDate) = React.useState(_ => {
    if selectedDate !== "" {
      ...
      let dateDay = date->Belt.Array.get(2)->Option.getOr("1")
      let dateMonth = date->Belt.Array.get(1)->Option.getOr("1")
      let dateYear = date->Belt.Array.get(0)->Option.getOr("1970")
      ...
      let timeHour = timeSplit->Belt.Array.get(0)->Option.getOr(currentDateHourFormat)
      let timeMinute = timeSplit->Belt.Array.get(1)->Option.getOr(currentDateMinuteFormat)
      let timeSecond = timeSplit->Belt.Array.get(2)->Option.getOr(currentDateSecondsFormat)
  ...
  let onDateClick = str => {
    ...
    let currentDateDay = currentDateSplit->Belt.Array.get(2)->Option.getOr("1")
    let currentDateYear = currentDateSplit->Belt.Array.get(0)->Option.getOr("1970")
    let currentDateMonth = currentDateSplit->Belt.Array.get(1)->Option.getOr("1")
    ...
    let startDateStr = if selectedDate === "" {
      ...
      `${selectedDate} ${time === ""
          ? `${currentDateHourFormat}:${currentDateMinuteFormat}${showSeconds
                ? ":" ++ currentDateSecondsFormat
                : ""}`
          : splitTime->Belt.Array.get(0)->Option.getOr("NA") ++
            ":" ++
            splitTime->Belt.Array.get(1)->Option.getOr("NA") ++ (
              showSeconds ? ":" ++ splitTime->Belt.Array.get(2)->Option.getOr("NA") : ""
            )}`
    }
    startDateStr
  }
