...
let make = (
  ...
  ~isFileUploadIconShow=true,
  ~primaryText="Drag and drop to add Image",
  ~secondaryText="or browse to choose a file",
) => {
  ...
  let style = switch customStyle {
  | Some(style) => style
  | None => `flex flex-col justify-center items-center border border-dashed rounded-2xl m-6 dark:border-jp-gray-960 h-full p-8`
  }
  ...
  <div
    ...
    onDrop={ev => {
      ...
      if files->Array.length > 0 {
        ...
        if size > 400000 {
          ...
          let fileReader = FileReader.reader
          let _file = fileReader.readAsDataURL(. file)
          fileReader.onload = e => {
            let target = ReactEvent.Form.target(e)
            let data = target["result"]
            setIsNewUpload(_ => true)
    ...
    className={style}>
    {isFileUploadIconShow ? <Icon name="file-upload" size=50 /> : React.null}
    <div className="text-xl font-bold text-gray-400 mt-4 mb-2"> {React.string(primaryText)} </div>
    <div className="text-xs font-semibold text-gray-400 mb-4"> {React.string(secondaryText)} </div>
