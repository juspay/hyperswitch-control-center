...
import * as CommonAuthHooks from "../Common/CommonAuthHooks.bs.js";
...
function BasicAuth(props) {
  ...
  var onSubmit = async function (values, param) {
    try {
      ...
      if (match) {
        switch (authType) {
          ...
          case "ResendVerifyEmail":
            var body = CommonAuthUtils.getEmailBody(
              email,
              undefined,
              undefined,
            );
            tmp = resendVerifyEmail(body);
            break;
          default:
            exit = 2;
        }
        ...
        switch (authType) {
          case "SignUP":
            ...
            var body$1 = CommonAuthUtils.getEmailPasswordBody(
              email,
              password,
              country,
            );
            tmp = getUserWithEmailPassword(body$1, "SIGNUP");
            break;
          case "LoginWithPassword":
          case "ResetPassword":
            exit = 1;
      ...
      switch (exit) {
        case 1:
          if (authType === "LoginWithPassword") {
            var password$1 = LogicUtils.getString(valuesDict, "password", "");
            var body$2 = CommonAuthUtils.getEmailPasswordBody(
              email,
              password$1,
              country,
            );
            tmp = getUserWithEmailPassword(body$2, "SIGNINV2");
          } else {
            ...
            var body$3 = CommonAuthUtils.getResetpasswordBodyJson(
              password$2,
              password_reset_token,
            );
            tmp = setResetPassword(body$3);
          }
          break;
        case 2:
          ...
          break;
        case 3:
          var body$5 = CommonAuthUtils.getEmailBody(email, country, undefined);
          tmp = getUserWithEmail(body$5);
          break;
      }
      await tmp;
    } catch (exn) {
  ...
  var note = CommonAuthHooks.useNote(
    authType,
    setAuthType,
  ...
  return JsxPPXReactSupportU.createElementWithKey(
    ...
    ReactFinalForm$1.Form,
    {
      ...
      validate: function (values) {
        return BasicAuthUtils.validateForm(values, validateKeys);
      },
    },
  );
}
