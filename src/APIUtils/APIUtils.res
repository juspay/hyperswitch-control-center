  ...
  let getUrl = (
    ...
    | V1(entityNameType) =>
      ...
      | OFFERS =>
        ...
        | #OFFER_UI_CONFIG =>
          switch (methodType, queryParameters) {
          | (Get, Some(queryParams)) => Euler(`${offersBaseURL}/ui/config?${queryParams}`)
          | (Get, None) => Euler(`${offersBaseURL}/ui/config`)
          | _ => Default("")
          }
        | #OFFER_CREATE =>
          switch (methodType, queryParameters) {
          | (Post, Some(queryParams)) => Euler(`${offersBaseURL}/create?${queryParams}`)
          | (Post, None) => Euler(`${offersBaseURL}/create`)
          | _ => Default("")
          }
        | #OFFER_UPDATE =>
          switch (methodType, id) {
          | (Put, Some(offerId)) => Euler(`${offersBaseURL}/${offerId}/v2/update`)
          | _ => Default("")
          }
        | #OFFER_UPDATE_CONFIG =>
          switch (methodType, id) {
          | (Get, Some(offerId)) => Euler(`${offersBaseURL}/${offerId}/update/config`)
          | _ => Default("")
          }
        | #OFFER_APPROVE =>
          switch (methodType, id) {
          | (Get, Some(offerId)) => Euler(`${offersBaseURL}/${offerId}/approve`)
          | _ => Default("")
          }
        | #OFFER_CODES =>
          switch (methodType, id) {
          | (Get, Some(offerId)) =>
            switch queryParameters {
            | Some(queryParams) => Euler(`${offersBaseURL}/${offerId}/codes?${queryParams}`)
            | None => Euler(`${offersBaseURL}/${offerId}/codes`)
            }
          | (Post, Some(offerId)) => Euler(`${offersBaseURL}/${offerId}/codes`)
          | (Delete, Some(offerId)) =>
            switch queryParameters {
            | Some(queryParams) => Euler(`${offersBaseURL}/${offerId}/codes?${queryParams}`)
            | None => Euler(`${offersBaseURL}/${offerId}/codes`)
        ...
        | #OFFER_CODES_GENERATE =>
          switch (methodType, id) {
          | (Post, Some(offerId)) => Euler(`${offersBaseURL}/${offerId}/codes/generate`)
          | _ => Default("")
          }
        | #OFFER_CSV_DOWNLOAD =>
          switch methodType {
          | Post => Euler(`${offersBaseURL}/csv-downloads`)
          | _ => Default("")
          }
        | #NONE => Default("")
        }
