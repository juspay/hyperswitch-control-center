...
test.describe("Profile - User Account Menu", () => {
  ...
  test("should open user account dropdown", async ({ page }) => {
    ...
    await expect(page.getByText('Regenerate recovery codesRegenerate your access code to ensure continued access and security for your account.')).not.toBeVisible();
    await expect(page.getByRole('button', { name: 'Edit' }).nth(1)).not.toBeVisible();
  });
...
test.describe("Profile Settings - Change Password", () => {
  ...
  test("should change password successfully and log the user out", async ({ page }) => {
    ...
    await expect(signInPage.headerText).toContainText("Hey there, Welcome back!");
    await expect(page.locator('div').filter({ hasText: /^Password Changed Successfully$/ }).nth(1)).toBeVisible();
  });
...
test.describe("Profile - Two Factor Authentication - TOTP", () => {
  ...
  test("should not prompt for current TOTP when TOTP is filled during login", async ({ page }) => {
    const signinPage = new SignInPage(page);
    ...
    await expect(page.getByText('Follow these steps to configure 2FA:1Scan the QR code with your authenticator')).toBeVisible();
  ...
  test("should prompt for TOTP when user skips 2FA during signin", async ({ page }) => {
    const homePage = new HomePage(page);
    const signInPage = new SignInPage(page);
  ...
  test("should scan the QR code and enter new OTP to reset TOTP", async ({ page }) => {
    const signinPage = new SignInPage(page);
    ...
    await expect(page.getByText("Successfully reset TOTP!")).toBeVisible({ timeout: 10000 });
  ...
  test("should show error and allow retry when verifying a wrong OTP in Reset TOTP flow", async ({ page }) => {
    const signinPage = new SignInPage(page);
    ...
    await page.getByRole('button', { name: 'Verify new OTP' }).click();
    await expect(page.locator('div').filter({ hasText: /^Invalid TOTP$/ }).nth(1)).toBeVisible({ timeout: 10000 });
    await expect(signinPage.qrCode2FA).toBeVisible();
    await expect(page.getByRole('button', { name: 'Verify new OTP' })).toBeVisible();
  });

  test("should reset TOTP using a stored recovery code after skipping 2FA at signin", async ({ page }) => {
    const homePage = new HomePage(page);
    const signInPage = new SignInPage(page);
    ...
    await expect(page.locator('input[name="recovery_code_input"]')).toBeVisible();

    await page.locator('input[name="recovery_code_input"]').fill(recoveryCodes[0]);
  ...
  test("should show error when entering a wrong recovery code after skipping 2FA at signin", async ({ page }) => {
    const homePage = new HomePage(page);
    const signInPage = new SignInPage(page);
    ...
    await expect(page.locator('input[name="recovery_code_input"]')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Verify recovery code' })).toBeVisible();
  });
...
test.describe("Profile - Two Factor Authentication - Recovery codes", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(signinPage.downloadRecoveryCodes).toBeVisible({ timeout: 10000 });
    await signinPage.downloadRecoveryCodes.click();
  ...
  test("should display, regenerate, and download fresh recovery codes", async ({ page }) => {
    const signinPage = new SignInPage(page);

    await page.getByRole('button', { name: 'Edit' }).nth(1).click();

    await expect(page).toHaveURL(/.*\/account-settings\/profile\/2fa\?type=regenerate_recovery_code/);
    await expect(page.getByText("Two factor recovery codes", { exact: true })).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("Recovery codes provide a way to access your account if you lose your device and can't receive two-factor authentication codes.")).toBeVisible();
    await expect(page.getByText('These codes are the last resort for accessing your account in case you lose your password and second factors. If you cannot find these codes, you will lose access to your account.')).toBeVisible();
    ...
    await expect(page.locator('[data-toast="Successfully regenerated new recovery codes!"]').first()).toBeVisible({ timeout: 10000 });
    await expect(page).toHaveURL(/.*\/account-settings\/profile(\?.*)?$/);

    await page.getByRole('button', { name: 'Edit' }).nth(1).click();
    await expect(signinPage.recoveryCodesMask).toBeVisible({ timeout: 10000 });
    await expect
      .poll(async () => (await signinPage.recoveryCodesMask.innerText()).trim(), { timeout: 10000 })
      .not.toBe(firstCodes);
  });

  test("should prompt for TOTP when accessing recovery codes after skipping 2FA during signin", async ({ page }) => {
    const homePage = new HomePage(page);
    const signinPage = new SignInPage(page);
