...
test.describe("Surcharge", () => {
  ...
  test.describe("Sidebar / Feature flag", () => {
    test("should expose the Surcharge menu under Workflows when surcharge flag is ON", async ({
      page,
    }) => {
    ...
    test("should hide the Surcharge menu when surcharge flag is OFF", async ({
      page,
    }) => {
      const homePage = new HomePage(page);
  ...
  test.describe("List view (LANDING)", () => {
    ...
    test("Edit icon switches the page into the form (pageView = NEW)", async ({
      page,
      context,
    }) => {
      const homePage = new HomePage(page);
    ...
    test("Delete icon opens the confirmation popup and removes the rule on Confirm", async ({
      ...
      await expect(
        page.getByText(/Successfully deleted current active surcharge rule/),
      ).toBeVisible();
  ...
  test.describe("Save flow", () => {
    test("Save fires PUT /routing/decision/surcharge and shows the success toast", async ({
      page,
      context,
    }) => {
      const homePage = new HomePage(page);
  ...
  test.describe("Rule list create", () => {
    ...
    test("should render all elements in surcharge create page", async ({
      page,
    }) => {
      const surcharge = new Surcharge(page);
      ...
      await expect(surcharge.ruleNameInput).toHaveValue(
        new RegExp(`^Surcharge -(${today}|${yesterday})$`),
      );

      // Surcharge section heading + helper copy
      await expect(
        page.getByText("Configure Advanced Rules to apply surcharges", {
          exact: true,
        }),
      ).toBeVisible();
      ...
      await expect(
        page.getByText(
          /Ensure to enter the payment amount and surcharge fixed amount in the smallest currency unit/,
        ),
      ).toBeVisible();
      await expect(
        page.getByText(
          /pass 100 to charge \$1\.00 \(USD\) and ¥100 \(JPY\) since ¥ is a zero-decimal currency/,
        ),
      ).toBeVisible();
      ...
      await expect(
        page.getByText("Surcharge is", { exact: true }),
      ).toBeVisible();
      await expect(page.getByRole("button", { name: "Rate" })).toBeVisible();
      await expect(
        page.getByText("Tax on Surcharge", { exact: true }),
      ).toBeVisible();
      await expect(surcharge.surchargeValueInput("percentage")).toHaveValue(
        "0",
      );
    ...
    test("switching between Rate and Fixed swaps the value field to amount and percentage input", async ({
      page,
    }) => {
      const surcharge = new Surcharge(page);
    ...
    test("should fill the form, save the rule, and preview it on /surcharge", async ({
      page,
    }) => {
      const surcharge = new Surcharge(page);
      ...
      await expect(
        previewer.getByText("Rate -> 5 | Tax on Surcharge -> 1", {
          exact: true,
        }),
      ).toBeVisible();
    });
