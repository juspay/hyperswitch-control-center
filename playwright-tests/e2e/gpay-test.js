...
(async () => {
  ...
  try {
    ...
    await (
      await sdkFrame.waitForSelector('[data-testid="addNewCard"]', {
        timeout: 10000,
        visible: true,
      })
    ).click();
    ...
    await gpayIframe.waitForSelector('[class="VfPpkd-vQzf8d"]', {
      timeout: 15000,
    });
    await gpayIframe.click('[class="VfPpkd-vQzf8d"]');

    await page.waitForSelector('[class="font-semibold text-xl capitalize"]', {
      timeout: 10000,
      visible: true,
    });
